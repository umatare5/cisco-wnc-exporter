...
- A controller response with a status of 400 or above has its body written to the log at error level, and the same body is carried in the error string this exporter logs in turn. An error response body is the controller's own error document rather than configuration data, and nothing redacts what is logged: each line carries at most the first 512 bytes, which the SDK marks with `... (truncated)`, and the whole document stays in memory on the typed error no log path reaches. The credential does not appear in either line.
