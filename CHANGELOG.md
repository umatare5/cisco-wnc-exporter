...
Two families report where the controller thinks each AP is, behind a new default-off flag `--collector.ap.geolocation`. They come from the geolocation operational container, which is a module of its own on the controller rather than part of the AP inventory, so the new flag reads it and nothing else — one request per refresh whose cost does not grow with the number of APs, and no existing configuration pays for it.

- `wnc_ap_longitude_degrees{mac}` and `wnc_ap_latitude_degrees{mac}` publish the WGS 84 pair as the controller reports it. **The unit is the platform's own**: `show ap geolocation summary` heads both columns with degrees and the schema the controller serves declares `units "degree"` on both leaves. They are two families rather than one carrying an axis label because longitude and latitude are different quantities over different domains, `±180` against `±90`, so an average or a sum across such a label denotes nothing.
- **The pair is published together or not at all.** A longitude without its latitude puts the AP on the prime meridian at whatever latitude a query joins in, which reads as a location rather than the gap it is, so both are withheld when either is missing, unparsable, or outside its own domain. The domain check is this exporter's own: the schema types both leaves as `decimal64` with no `range` statement, so a value beyond `±180` is well formed on the wire.
- **It is a configured position rather than a measurement.** All three APs on the estate this was read against reported their location source as manual, and the statistics container counted zero APs with a GNSS module and zero with a derived position — so the value changes when someone changes it. A `reset capwap` was observed losing it, which is why a joined AP can lose the series.
- **An estate with no coordinates configured costs nothing but the request.** The container answered `200` on IOS-XE 17.12 and `204` on 17.15, 17.18 and 26.01, and a `204` is a successful read of nothing, so both series are simply absent and `wnc_refresh_errors_total{data="ap_geo_loc_data"}` stays at zero. It answered `404` on none of the four.

Two opt-in labels join `wnc_client_info`. They read leaves the collector already has in hand, so they add no request, no data type, no descriptor and no failure domain, and the default label set is unchanged — a deployment that does not name them keeps every series byte for byte.

- `wlan_id` carries the WLAN identifier of the client's association, from the same leaf `wnc_wlan_clients` buckets by, so `count by (wlan_id) (wnc_client_info)` reconciles with `wnc_wlan_clients{id}` by construction. **It is not named `id`**: `wnc_wlan_info` already carries the identifier under that name and an `id` on a client series cannot say whose it is, so joining the two takes `label_replace` — the [Client](docs/collector.client.md) page gives the expression. It is empty rather than `0` where the controller omits the leaf, because no WLAN carries identifier `0`.
- `device_type` carries the device class the controller assigned. **It is the controller's conclusion rather than the client's claim**, and the confidence leaf beside it read `0` on most records measured, so read a change as a reclassification. **It moves the series identity of `wnc_client_info`**, which the info cache holds for `--collector.info-cache-ttl`, so a reclassification can leave two series for one `mac` and a `group_left` over them fails the whole query rather than taking the newer one. That is the hazard the `ap` label already carries, and asking for both puts two churning labels on one series.

### Fixed

Two panels of the bundled admin dashboard filtered `wnc_client_info` on its `wlan` label, which carries the SSID name, using the `wlan_id` template variable, which holds the numeric identifiers of `wnc_wlan_info`. Selecting a WLAN returned an empty panel. Both now filter on the new `wlan_id` label, so the variable and the label name the same quantity.

The controller boot instant now goes through the SDK's own accessor instead of a path this exporter built itself, so the decode moves upstream and one hand-written RESTCONF literal is gone. **The published series is unchanged** — same name, same type, same value, withheld under the same two conditions. What changes is where a malformed instant surfaces: it used to be a debug line and a silent absence, and it is now a read failure, so `wnc_refresh_errors_total{data="controller_boot_time"}` gains a way to climb while the series is withheld exactly as before. The metrics alone cannot tell that case from a leaf the controller simply omitted, which is why the counter matters. Keep `controller_boot_time` in any exclusion the [Controller](docs/collector.controller.md) page's note \*4 suggests. One bundled alert changes which rule catches the case rather than whether it is caught: `wnc_refresh_items{data="controller_boot_time"}` goes absent on a failed read instead of reading `1`, so `WNCControllerBootTimeMissing` no longer matches a malformed instant and the unlabelled `WNCDataTypeFailing` does — at `warning` after five minutes rather than after fifteen. Point a runbook at both.
