...
`wnc_wlan_policy_enabled` reads a leaf that carries no absent value, so an omitted leaf would report as shut down. On the controller this was measured against the leaf arrives on a plain read as well as on a read for the values in force, so a fallback does not fabricate one — keep `unless on(job, instance) increase(wnc_refresh_defaults_fallback_total[15m]) > 0` on a rule anyway, since another controller was not measured. The series is absent, and any rule on it silent, for a WLAN that resolves to no policy profile and whenever the `wlan_policies` or `wlan_policy_list_entries` fetch fails, so pair it with `absent()` or watch `wnc_refresh_items` for those two `data` types. Whether a shut policy profile also stops the SSID being advertised is not established, so treat the series as change detection rather than an outage signal.
