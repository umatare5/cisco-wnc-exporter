...
- The [Overview](docs/README.md) page said a counter reset can reach an AP that went nowhere, and note \*4 on the [AP](docs/collector.ap.md) page said the channel-change counter falls to zero on every radio at once. Re-measured against an access point left untouched as a control, neither holds: the untouched AP kept every counter and every anchor, and the count fell only on the radio that had one to lose. **A per-radio counter is anchored at its own AP's CAPWAP join**, so a rule watching one AP does not see a reset caused elsewhere. The boot time is not the anchor and cannot be told from the join, and two radios of one AP can anchor at different instants. The v0.9.0 entry below drew the withdrawn conclusion and is corrected in place.
