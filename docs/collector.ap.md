...
Every series of this module carries `mac` and nothing else, deliberately. A bare `and` requires both sides to carry identical label sets, so one extra label on either side makes the query below return an empty result rather than an error:
...
Two cautions on that query. An AP that holds this controller as its secondary, or that discovers by broadcast, DHCP option 43 or DNS, sends discovery requests here while joining another controller, so pair the rule with a `for:` longer than the rate window and exclude those APs by `mac`. And a counter here is cumulative since the controller allocated the record, so `increase()` over a range shorter than the discovery interval reports nothing.

`wnc_ap_join_info` carries the name because the inventory no longer does: `ap-join-info` is the only container that still names a departed AP, which is why the name is a series of its own rather than a label on the other thirty-one. Join it with `* on(mac) group_left(name) wnc_ap_join_info`, which needs no collapsing because it carries no `radio` label. Its name puts it in the info cache, and that cache is only wrapped around the collector when `--collector.ap.info` is enabled, so with the join module alone the name is read fresh on every scrape and with both modules enabled it is up to `--collector.info-cache-ttl` old.

The record set is wider than the inventory, so `count(wnc_ap_joined)` can exceed `count(wnc_ap_info)`. What evicts a record is not established — leaving CAPWAP does not, and no controller reload has been observed against this list.
