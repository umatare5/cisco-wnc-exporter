...
- `wnc_ap_channel_changes_total{mac,radio}` counts the channel changes the controller records for a radio. It comes from the same record as the RRM verdicts, so it joins the AP `radio` module and adds no request. **It does not separate DCA, DFS and manual assignment** — compare it with `wnc_ap_last_radar_timestamp_seconds` to tell a radar-driven move from an RRM one.
